...
@click.option(
    ...
    default=True,
    show_default="True",
    help="Whether to drop the current database",
)
@click.option(
    "--email/--no-email",
    default=True,
    show_default="True",
    help="Whether to send emails along with the alert",
)
@click.option(
    "--slackbot/--no-slackbot",
    default=True,
    show_default="True",
    help="Whether to send the alert on slack",
)
def run_coincidence(firedrill, dropdb, email, slackbot):
    """Initiate Coincidence Decider"""

    HOST = gethostname()
    coinc = snews_coinc.CoincidenceDistributor(
        drop_db=dropdb,
