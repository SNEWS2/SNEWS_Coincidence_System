...
def get_image(is_test, alert_data, topic):
    ...
    updatelink = (
        "https://www.shutterstock.com/image-vector/"
        "vector-illustration-modern-label-new-600w-1520423249.jpg"
    )
    # updatelink = ("https://raw.githubusercontent.com/SNEWS2/"
    #               "SNEWS_Coincidence_System/main/snews_cs/etc/update_image.png")
    # "https://www.ris.world/wp-content/uploads/2018/09/update.jpg"
    sendlink = (
        giflink
        if alert_type == "NEW_MESSAGE"
        else (updatelink if alert_type == "UPDATE" else retractlink)
    )
...
# The `text` argument is used in places where content cannot be rendered such as:
...
def send_table(alert_data, alert, is_test, topic):
    ...
    try:
        ...
        log.info("We ran into slack connection problems. slack message DID NOT go out.")
